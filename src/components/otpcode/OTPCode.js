...
const OTPCode = () => {
  const { setotp, posphno, userid, roles, setroles } = useContext(Master);
  ...
  const handleverifyotp = () => {
    ...
    console.log('Verifying OTP:', enteredOtp);
      ...
      axios.get(`http://localhost/asiatech/back?rest_route=/simple-jwt-login/v1/auth&phone_number=${posphno.current}&secod=${enteredOtp}&shakti=${userid.current}`)
      .then((response) => {
